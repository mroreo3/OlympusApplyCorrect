...
class Life_Settings {
	...
	debit_tax = false;	//Tax a player when they have used a Debit card to purchase something? - This is mainly a money sink in case everyone on your server owns a debit card, as they'll never have money on their person (so they can't be robbed) if they do.
	debit_taxAmount = 0; // Tax the player pays when purchasing with a debit card. Tax = Price * debit_taxAmount. debit_tax must be true. Eg. - If an item is $45,000 and your tax is set to 0.15, amount paid after tax will be $51,750. (total tax paid $6750.)
	debit_price = 1; // Price of purchasing a debit card from the bank.
...
/* System Settings */
    ...
    fuel_cost = 0; //Cost of fuel per liter at fuel stations (if not defined for the vehicle already).
    service_chopper = 0; //Cost to service chopper at chopper service station(Repair/Refuel).
    fuelCan_refuel = 0; //Cost to refuel an empty fuel canister at the fuel station pumps. (Be wary of your buy/sell prices on fuel cans to prevent exploits...)
    ...
    vehicle_purchase_multiplier_MEDIC = 1.5; //Medic Vehicle Buy Price = Config_Vehicle price * multiplier
